...
public class JwtService {
    ...
    public JwtService(VaultProperties properties, TokenBlacklistService tokenBlacklistService) {
        this.signingKey = buildSigningKey(properties.getJwt().getSecret());
        this.accessTokenExpirationMs = properties.getJwt().getExpirationMs();
        this.refreshTokenExpirationMs = properties.getJwt().getRefreshExpirationMs();
    ...
    private String generateToken(VaultUser user, String tokenType, long expirationMs) {
        ...
        return Jwts.builder()
                .claims(Map.of(
                        ...
                        CLAIM_TENANT_ID, user.getTenantId(),
    ...
    private SecretKey buildSigningKey(String jwtSecret) {
        if (jwtSecret == null || jwtSecret.isBlank()) {
            throw new IllegalStateException("vault.jwt.secret must be configured");
        }

        byte[] keyBytes = jwtSecret.getBytes(StandardCharsets.UTF_8);
        if (keyBytes.length < 32) {
            throw new IllegalStateException("vault.jwt.secret must be at least 256 bits for HS256");
        }
